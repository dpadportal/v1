...
import { generateArtaReference, isUniqueConstraintError } from "../src/lib/arta";
import { applySchema } from "./helpers";

describe("generateArtaReference", () => {
  beforeAll(async () => {
    await applySchema();
  });
  ...
  it("starts at 00001 for the current year", async () => {
    ...
    expect(await generateArtaReference(env.DB)).toBe(`ARTA-${year}-00001`);
  });
  ...
  it("increments after existing tickets", async () => {
    await env.DB.prepare(
      ...
      .bind(`ARTA-${new Date().getFullYear()}-00007`)
      .run();

    const year = new Date().getFullYear();
    expect(await generateArtaReference(env.DB)).toBe(`ARTA-${year}-00008`);
  });
});

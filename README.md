...
- **Android APK:** `/dpac-portal.apk` (1.3 MiB, package id `ne.dpacportal.app`, launcher name **DPAD**). It's a signed Trusted Web Activity (TWA) built through the [PWABuilder CloudAPK service](https://pwabuilder-cloudapk.azurewebsites.net/) (`POST /enqueuePackageJob` → poll `/getPackageJob?id=` → download `/downloadPackageZip?id=`).

...
- **PWA:** `/manifest.webmanifest` + `/sw.js` (offline shell) + icon set under `/icons/`. A "Get the DPAD app" pill appears on mobile — Chrome/Edge Android install natively, iPhone uses Safari Share → *Add to Home Screen*.
- **Android APK:** `/dpac-portal.apk` (1.0 MiB, package id `ne.dpacportal.app`, launcher name **DPAD**). It's a signed Trusted Web Activity (TWA) built through the [PWABuilder CloudAPK service](https://pwabuilder-cloudapk.azurewebsites.net/) (`POST /enqueuePackageJob` → poll `/getPackageJob?id=` → download `/downloadPackageZip?id=`).
